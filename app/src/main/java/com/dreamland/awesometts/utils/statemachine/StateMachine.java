...
package com.dreamland.awesometts.utils.statemachine;
...
class Hsm1 extends StateMachine {
    ...
    public static Hsm1 makeHVoiceStateMachine() {
        log("makeHVoiceStateMachine E");
        Hsm1 sm = new Hsm1("hsm1");
        sm.start();
        log("makeHVoiceStateMachine X");
        return sm;
    }
...
<code>
Hsm1 hsm = makeHVoiceStateMachine();
synchronize(hsm) {
 ...
 * <p>The output is:</p>
<code>
D/hsm1    ( 1999): makeHVoiceStateMachine E
D/hsm1    ( 1999): ctor E
D/hsm1    ( 1999): ctor X
D/hsm1    ( 1999): mP1.enter
D/hsm1    ( 1999): mS1.enter
D/hsm1    ( 1999): makeHVoiceStateMachine X
...
public class StateMachine {
    ...
    private static class SmHandler extends Handler {
        ...
        private IState getCurrentState() {
            return mStateStackTopIndex == -1 ? null: mStateStack[mStateStackTopIndex].state;
        }
        ...
        /** @see StateMachine#deferSameMessage(Message) */
        private void deferSameMessage(Message msg) {
            if (mDbg) mSm.log("deferSameMessage: msg=" + msg.what);

            /* Copy the "msg" to "newMsg" as "msg" will be recycled */
            Message newMsg = obtainMessage();
            newMsg.copyFrom(msg);
            Message target = null;
            for (Message next : mDeferredMessages) {
                if (next.what == msg.what) {
                    target = next;
                    break;
                }
            }
            if(target != null){
                mDeferredMessages.remove(target);
            }
            mDeferredMessages.add(newMsg);
    ...
    /**
     * Defer this message and remove last same message until next state transition.
     * Upon transitioning all deferred messages will be
     * placed on the queue and reprocessed in the original
     * order. (i.e. The next state the oldest messages will
     * be processed first)
     ...
     * @param msg is deferred until the next transition.
     */
    protected final void deferSameMessage(Message msg){
        mSmHandler.deferSameMessage(msg);
    }
